...
if [ -z "$database_name" ]; then
  echo "POSTGRES_DB is missing in the postgres container." >&2
  exit 1
fi
...
docker compose -f "$compose_file" exec -T postgres sh -c \
  'export PGPASSWORD="$POSTGRES_PASSWORD"; exec pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc -Z 9' \
  > "$local_path"
...
find "$backup_dir" -type f -name "${database_name}-*.dump" -mtime +"$LOCAL_RETENTION_DAYS" -delete
