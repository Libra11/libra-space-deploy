...
admin_image="\$(awk -F= '/^ADMIN_IMAGE=/ {print substr(\$0, index(\$0, \$2)); exit}' .env)"
if [ -z "\$admin_image" ]; then
  echo "ADMIN_IMAGE missing in /opt/libra-space/.env" >&2
  exit 1
fi
...
echo "Deploying server image on ECS: $ECS_INSTANCE_ID"
invoke_id="$(
  ...
  "$aliyun_cli" ecs RunCommand \
    ...
    --Timeout 900 \
    --Name "release-libra-space-server-$VERSION" \
    --KeepCommand false \
